...
## [Live Web Panel](https://library-app.duckdns.org/app/admin/loans)
- [Web Panel Instructions](./docs/prod_instructions.md)
...
As the project evolves, the following features are planned for implementation to enhance scalability, performance, and user experience:

- **AI-Powered Recommendation System**: A specialized Python microservice utilizing Machine Learning (Scikit-learn) to provide personalized book suggestions based on user borrowing patterns and preferences.
- **Dynamic Reporting Module**: Automated generation of comprehensive financial and analytical reports in PDF and XLSX formats to support administrative decision-making and data portability.
- **Advanced Caching Layer**: Leveraging Redis to cache high-frequency read requests, aiming to drastically reduce API response times and decrease database load.
- **ISBN Integration & Metadata Automation**: Integration with external APIs (like Google Books) to allow instant book registration via ISBN scanning, including automatic fetching of covers and descriptions.
